...
\begin{solution}
  ...
  \item $A_1 = X\setminus g(Y)$ is all of $X$ that is not mapped onto by $Y$. If $A_1=\emptyset$, then $g$ is a surjection and $X\sim Y$ is already proven. Now $A_{n+1}=g(f(A_n))$. $A_1$ and $A_2$ are disjoint because $A_2$ is mapped onto by $Y$, and \TODO
  }
\end{solution}