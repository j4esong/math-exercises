...
\begin{solution}
  ...
  We are given two conditions: first, for any $m$, there exists $S_m$ (dependent on $m$) such that
  $$
  |a_{mn} - b_m| < \epsilon/2 \text{ for } n\geq S_m.
  $$
  Second, there exists $S$ such that
  $$
  |a_{mn} - a|<\epsilon/2 \text{ for } m, n\geq S.
  $$
  This implies that, for all $m\geq S$, there exists $S_m$ such that the first condition is true; then choose $n\geq \max(S_m, S)$, and we have shown that for any choice of $m\geq S$, there exists an $n$ such that both conditions are true. Thus we can say
  $$
  |b_m-a|\leq|b_m-a_{mn}|+|a_{mn}-a|<\epsilon \text { for } m \geq S.
  $$
  \item 
  \BAD
